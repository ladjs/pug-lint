...
Linter.prototype = {
  checkDirectory(directoryPath) {
    ...
    for (const file of fs.readdirSync(directoryPath)) {
      ...
      if (stat.isDirectory()) {
        errors = [...errors, ...this.checkDirectory(filePath)];
      } else if (this._fileExtensions.includes(path.extname(filePath))) {
        errors = [...errors, ...this.checkFile(filePath)];
      }
    }
  ...
  configure(options) {
    ...
    if (options) {
      ...
      if (options.preset) {
        throw new Error(
          'Presets have been deprecated. See: https://github.com/pugjs/pug-lint#preset-deprecated'
        );
      }
      ...
      if (options.excludeFiles) {
        assert(
          ...
          '"excludeFiles" option requires array value'
        );
      ...
      if (options.additionalRules) {
        assert(
          Array.isArray(options.additionalRules),
          '"additionalRules" option requires array value'
        );

        this._ruleMasks = [...this._ruleMasks, ...options.additionalRules];
      }
          ...
          if (options[name] && options[name] !== null) {
            if (rule.contradictions) {
              for (const contradiction of rule.contradictions) {
                if (options[contradiction] && options[contradiction] !== null) {
  ...
  _loadExcludedFiles() {
    this._excludedFileMatchers = this._excludedFileMasks.map((fileMask) => {
  ...
  _resolveExtendsFile(filePath) {
    if (isAbsolutePath(filePath) || !/\w|@/.test(filePath.charAt(0))) {
      ...
      if (!fs.existsSync(filePath)) {
        throw new Error(
          'Cannot find configuration file "' + filePath + '" to extend'
        );
      }
    } else {
      const packageName = utils.normalizePackageName(
        filePath,
        'pug-lint-config'
      );
