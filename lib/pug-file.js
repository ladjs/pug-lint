...
class PugFile {
  ...
  addErrorForIncorrectSpacesAroundFilter(options, errors) {
    this.iterateTokensByFilter(options.filter, (token) => {
      const separator = ' ';
      let columnIndex;
      let columnNumber;
      const errorMessage = [
      ...
      switch (options.position) {
        case 'before':
          ...
          break;
        default:
          columnIndex = token.col + tokenWidth;
      ...
      const match = this.getCharacter(token.line, columnIndex);

      this.addErrorForMatch(
        match,
        separator,
        !options.required,
        errors,
  ...
  addErrorForIncorrectTokenTypeOrder(
    ...
    this.iterateTokensByType(typeB, (token) => {
      const current = token;
      const currentIndex = current._index;
      const start =
        this.getPreviousTokenByType(token, boundaryTypes) || current;
      const startIndex = start._index;
      const end = this.getNextTokenByType(token, boundaryTypes);
  ...
  addErrorForCodeOperator(types, isRequired, errors) {
    ...
    for (const type of types) {
      ...
      this.addErrorForIncorrectSpacesAroundFilter(
        {
