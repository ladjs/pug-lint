...
module.exports.prototype = {
  ...
  configure(options) {
    assert(
      ...
      this.name + ' option requires string or object value or should be removed'
    );
    ...
    assert(
      ...
      this.name +
        ' option.separator requires string value containing only commas or spaces'
    );

    assert(
      typeof options.multiLineSeparator === 'string' &&
        /^([ ,\t\r\n]+)$/.test(options.multiLineSeparator),
      this.name +
        ' option.multiLineSeparator requires string value containing only commas, spaces or tabs'
    );
    ...
    file.iterateTokensByType('start-attributes', function (token) {
      ...
      if (endIndex > startIndex + 2) {
        file.iterateTokensByFilter(
          ...
          function (token) {
            const value =
              typeof token.val === 'boolean' ? '' : token.val.trim();

            patterns.push(
              getEscapedPattern(token.name) +
                (value.length > 0 ? '\\s*\\!*=\\s*' : '') +
        ...
        while ((match = regexFind.exec(parsedSource)) !== null) {
          ...
          current = file.getNextTokenByType(current, [
            'attribute',
            'end-attributes'
          ]);
