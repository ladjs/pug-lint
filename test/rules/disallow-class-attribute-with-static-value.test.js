  ...
  describe('disallowClassAttributeWithStaticValue', function () {
    describe('true', function () {
      ...
      it('should not report class attribute with dynamic value', function () {
        assert.equal(
          linter.checkString("a(class={active: currentUrl === '/'})").length,
          0
        );
      });
      ...
      it('should report multiple errors found in file', function () {
        const result = linter.checkFile(
          fixturesPath + 'disallow-class-attribute-with-static-value.pug'
        );

        assert.equal(result.length, 5);
        assert.equal(
          result[0].code,
          'PUG:LINT_DISALLOWCLASSATTRIBUTEWITHSTATICVALUE'
        );
