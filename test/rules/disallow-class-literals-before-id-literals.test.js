  ...
  describe('disallowClassLiteralsBeforeIdLiterals', function () {
    describe('true', function () {
      ...
      it('should report class literals before ID literals', function () {
        assert.equal(
          linter.checkString("input.class#id(type='text')").length,
          1
        );
      });
      ...
      it('should report multiple errors found in file', function () {
        const result = linter.checkFile(
          fixturesPath + 'disallow-class-literals-before-id-literals.pug'
        );

        assert.equal(result.length, 2);
        assert.equal(
          result[0].code,
          'PUG:LINT_DISALLOWCLASSLITERALSBEFOREIDLITERALS'
        );
