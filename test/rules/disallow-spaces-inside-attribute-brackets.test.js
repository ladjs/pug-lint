  ...
  describe('disallowSpacesInsideAttributeBrackets', function () {
    describe('true', function () {
      ...
      it('should report spaces inside attribute brackets', function () {
        assert.equal(
          linter.checkString("input( type='text' name='name' checked )").length,
          2
        );
      });
      ...
      it('should report multiple errors found in file', function () {
        const result = linter.checkFile(
          fixturesPath + 'disallow-spaces-inside-attribute-brackets.pug'
        );

        assert.equal(result.length, 10);
        assert.equal(
          result[0].code,
          'PUG:LINT_DISALLOWSPACESINSIDEATTRIBUTEBRACKETS'
        );
