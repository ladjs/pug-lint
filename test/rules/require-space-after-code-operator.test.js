    ...
    describe('true', function () {
      ...
      it('should report multiple errors found in file', function () {
        const result = linter.checkFile(
          fixturesPath + 'require-space-after-code-operator.pug'
        );
        ...
        assert.equal(
          result[0].msg,
          'One space required after unbuffered code operator'
        );
        assert.equal(result[0].line, 9);
        assert.equal(result[0].column, 2);
        assert.equal(
          result[3].msg,
          'One space required after buffered code operator'
        );
        assert.equal(result[3].line, 1);
        assert.equal(result[3].column, 3);
        assert.equal(
          result[7].msg,
          'One space required after unescaped buffered code operator'
        );
